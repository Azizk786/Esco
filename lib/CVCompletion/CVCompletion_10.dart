...
class CVCompletion_10 extends StatefulWidget {
  @override
  UserType  StrUserType;
  CVCompletion_10({Key key, @required this.StrUserType}) : super(key: key);
  _CVCompletion_10State createState() => _CVCompletion_10State(StrUserType);
}

class _CVCompletion_10State extends State<CVCompletion_10> {
  UserType  StrUserType;
  String struserid;
  bool _saving = false;
  Map userdata;
  ...
  List<String> _users = ["National","Regional","Other","Selectați certificatul",];
  ...
  _CVCompletion_10State(this.StrUserType);
  SaveCreerPlan_PersonalInfo() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("CV10", json.encode(_cv10));
  }
  ...
  bool createPayload()
  {
    ...
    _cv10 = {
      "diplome":_controllers,
      "procesareInformatie":_Procesarea,
      "comunicare":_Comunicare,
      "continut":_Creare,
      "securitate":_Securitate,
  "rezolvareProbleme":_Rezolvarea,
  ...
  Future<Null> GetCVPlandata() async
  {
    ...
    plandata = json.decode(
        sem_data["competente_digitale"].replaceAll(new RegExp(r"(\s\n)"), ""));

    print("print plan data = $plandata");

    setState(() {
      _Procesarea = plandata["procesareInformatie"];
      _Comunicare = plandata["comunicare"];
      _Creare = plandata["continut"];
      _Securitate = plandata["securitate"];
      _Rezolvarea = plandata["rezolvareProbleme"];
      _descr.text = plandata["competente"];
      List furar = plandata["diplome"];
      print("ppppp = $furar");
      String s = furar.join(', ');
  ...
  Widget Dialogview()
  {
    return  Container
      ...
      child: ListView.separated(
        shrinkWrap: true,
        itemCount: QuesionArr.family1.length,
          ...
          return FlatButton(
            onPressed: (){
              ...
              setState(() {
                if(_valueOf == 1)
                {
                  _Procesarea =  QuesionArr.family1[index][0];//+family[index][1];
                }
                else  if(_valueOf == 2)
                {
                  _Comunicare =  QuesionArr.family1[index][0];//+family[index][1];
                }
                else  if(_valueOf == 3)
                {
                  _Creare =  QuesionArr.family1[index][0];//+family[index][1];
                }
                else  if(_valueOf == 4)
                {
                  _Securitate =  QuesionArr.family1[index][0];//+family[index][1];

                }
                else  if(_valueOf == 5)
                {
                  _Rezolvarea =  QuesionArr.family1[index][0];//+family[index][1];
                }
            ...
            child:  RichText(
              ...
              text: TextSpan(
                children: <TextSpan>[
                  TextSpan(text:  QuesionArr.family1[index][0], style:
                  TextStyle(color: Appcolor.redHeader, fontSize: 16.0, fontFamily: 'dami')),
                  TextSpan(text:  QuesionArr.family1[index][1],style:
              ...
              children: <Widget>[
                ...
                Column(
                  ...
                  children: <Widget>[
                    ...
                    });},child: Text(_Procesarea,overflow: TextOverflow.ellipsis,
                        style:
                    ...
                    FlatButton(onPressed: (){ setState(() {
                      _valueOf = 2; _ackAlert(context);

                    });},child: Text(_Comunicare,overflow: TextOverflow.ellipsis,style:
                    TextStyle(color: Colors.black87, fontSize: 18.0, fontFamily: 'regular')),),
                    aGreyHorizonalLine,
                    SizedBox(height: 5,),
                    Text(
                    ...
                    FlatButton(onPressed: (){ setState(() {
                      _valueOf = 3; _ackAlert(context);

                    });},child: Text(_Creare,overflow: TextOverflow.ellipsis,style:
                    TextStyle(color: Colors.black87, fontSize: 18.0, fontFamily: 'regular')),),
                    aGreyHorizonalLine,
                    SizedBox(height: 5,),
                    Text(
                    ...
                    FlatButton(onPressed: (){ setState(() {
                      _valueOf = 4; _ackAlert(context);

                    });},child: Text(_Securitate,overflow: TextOverflow.ellipsis,style:
                    TextStyle(color: Colors.black87, fontSize: 18.0, fontFamily: 'regular')),),
                    aGreyHorizonalLine,
                    SizedBox(height: 5,),
                    Text(
                    ...
                    FlatButton(onPressed: (){ setState(() {
                      _valueOf = 5; _ackAlert(context);

                    });},child: Text(_Rezolvarea,overflow: TextOverflow.ellipsis,style:
                    TextStyle(color: Colors.black87, fontSize: 18.0, fontFamily: 'regular')),),
                    aGreyHorizonalLine,
                    SizedBox(height: 5,),
                  ],
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        "Certificatele TIC",
                        style:
                        TextStyle(color: Colors.black54, fontSize: 18.0, fontFamily: 'demi'),textAlign: TextAlign.left,
                      ),
                      SizedBox(height: 20,),
                  Text(
                  "Certificat national",
                  style:
                  TextStyle(color: Colors.grey, fontSize: 16.0, fontFamily: 'demi'),textAlign: TextAlign.left,
                ),
                      ListView.builder(
                        shrinkWrap: true,
                        itemCount: rowCount,
                      ...
                      SizedBox(height: 20,),
                      Text(
                        "Alte competente digitale",
                        style:
                        TextStyle(color: Colors.black38, fontSize: 18.0, fontFamily: 'demi'),textAlign: TextAlign.left,
                      ),
                      SizedBox(height: 10,),
                      TextFormField(
                          style: TextStyle(color: Colors.black54, fontSize: 18.0, fontFamily: 'demi'),
                          controller: _descr,
                  ...
                  child: Stack(
                    children: <Widget>[
                      Align(
                          ...
                          child: Text(
                              "Salvați și mergeți la pasul 11", style: TextStyle(fontFamily: 'regular')
                          )
                      )
                    ],
                  ),
