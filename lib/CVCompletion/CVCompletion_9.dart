...
class CVCompletion_9 extends StatefulWidget {
  @override
  UserType  StrUserType;
  CVCompletion_9({Key key, @required this.StrUserType}) : super(key: key);
  _CVCompletion_9State createState() => _CVCompletion_9State(StrUserType);
}

class _CVCompletion_9State extends State<CVCompletion_9> {
  UserType  StrUserType;
  String struserid;
  bool _saving = false;
  Map userdata;
  ...
  bool createPayload()
  {
    if(_descr.text.length <1) {
      ...
      showMyDialog(context,"Competente dobandite la locul de munca");
      return false;
    }
    _cv9 = {
  ...
  Future<Null>  GetCVPlandata() async
  {
    ...
    plandata = json.decode(
        sem_data["competente_loc_de_munca"].replaceAll(new RegExp(r"(\s\n)"), ""));
                  ...
                  child: Stack(
                    children: <Widget>[
                      Align(
                          ...
                          child: Text(
                              "Salvați și mergeți la pasul 10", style: TextStyle(fontFamily: 'regular')
                          )
                      )
                    ],
                  ),
