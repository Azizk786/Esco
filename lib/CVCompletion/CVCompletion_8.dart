...
class CVCompletion_8 extends StatefulWidget {
  @override
  UserType  StrUserType;
  CVCompletion_8({Key key, @required this.StrUserType}) : super(key: key);
  _CVCompletion_8State createState() => _CVCompletion_8State(StrUserType);
}

class _CVCompletion_8State extends State<CVCompletion_8> {
  UserType  StrUserType;
  String struserid;
  bool _saving = false;
  Map userdata;
  ...
  _CVCompletion_8State(this.StrUserType);
  SaveCreerPlan_PersonalInfo() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("CV8", json.encode(_cv8));
  }
  ...
  bool createPayload()
  {
    if(_descr.text.length <1) {
      ...
      showMyDialog(context,"Competente organizationale/manageriale");
      return false;
    }
    _cv8 = {
  ...
  Future<Null>  GetCVPlandata() async
  {
    ...
    plandata = json.decode(
        sem_data["competente_manageriale"].replaceAll(new RegExp(r"(\s\n)"), ""));
                  ...
                  child: Stack(
                    children: <Widget>[
                      Align(
                          ...
                          child: Text(
                              "Salvați și mergeți la pasul 9", style: TextStyle(fontFamily: 'regular')
                          )
                      )
                    ],
                  ),
