...
class CareerPlan_6 extends StatefulWidget {
  @override
  UserType  StrUserType;
  CareerPlan_6({Key key, @required this.StrUserType}) : super(key: key);
  _CareerPlan_6State createState() => _CareerPlan_6State(StrUserType);
}

class _CareerPlan_6State extends State<CareerPlan_6> {
  double _value = 0.7;
  UserType  StrUserType;
  //Map _payload;
  //
  ...
  _CareerPlan_6State(this.StrUserType);
  @override
  final _q1 = TextEditingController();
  bool _saving = false;
  String struserid;

  SaveCreerPlan_step6() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("CP6", json.encode(_cp6));
  }
  ...
  bool createPayload()
  {
    ...
    if(_q1.text.length <1) {
      ...
      showMyDialog(context,"Descrie-te in cateva popoziti");
      return false;
    }

    Map Sem_Payload =  {
      "continut":_q1.text.toString(),

    };

    _cp6 = {
      "1566389223572":Sem_Payload,
    };
  ...
  Future<Null>  GetCVPlandata1() async
  {
    ...
    Map plandata = json.decode(
        sem_data["descriere"].replaceAll(new RegExp(r"(\s\n)"), ""));
    setState(() {
      _q1.text = plandata["1566389223572"]["continut"];
              ...
              child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        Text("Descrie-te in cateva popozitii",
                            style:
                            TextStyle(color: Colors.grey, fontSize: 16.0, fontFamily: 'regular')),
                        TextFormField(
                          maxLines: 5,
                    ...
                    RaisedButton(
                      ...
                      onPressed: (){
                        if(StrUserType == UserType.Student || StrUserType == UserType.Pupil)
                        {
                          if (createPayload() == true)
                          {
//                            _apiTo_addCareerPlan6().then((res) {
//                              if (res == true)
//                              {
                                setState(() {
