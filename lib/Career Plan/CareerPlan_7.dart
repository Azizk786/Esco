...
class CareerPlan_7 extends StatefulWidget {
  @override
  UserType  StrUserType;
  CareerPlan_7({Key key, @required this.StrUserType}) : super(key: key);
  _CareerPlan_7State createState() => _CareerPlan_7State(StrUserType);
}

class _CareerPlan_7State extends State<CareerPlan_7> {
  double _value = 0.7;
  UserType  StrUserType;
//  Map _payload;
  Map _c10;
  ...
  _CareerPlan_7State(this.StrUserType);
  ...
  SaveCreerPlan_viziune_dezvoltare_personala() async {
    ...
    prefs.setString("CP7", json.encode(_cp7));
  }
  ...
  bool createPayload()
  {
    ...
    if(_q1.text.length <1) {
      ...
      showMyDialog(context,"Formuleaza-ti o misiune");
      return false;
    }

    Map Sem_Payload =  {
      "viziune":_q1.text.toString(),

    };
/*
{"1531295674019":{"viziune":"<p>Misiunea profesionala<\/p>\r\n"}}
* */

    _cp7 = {
      "1531295674019":Sem_Payload,
    };
  ...
  Future<Null> GetCVPlandata1() async
  {
    ...
    Map plandata = json.decode(
        sem_data["viziune_dezvoltare_personala"].replaceAll(new RegExp(r"(\s\n)"), ""));
    setState(() {
      _q1.text = plandata["1531295674019"]["viziune"];
//      List alte = plandata["1531295674019"]["alteObiectiveDirectii"];
//
//      rowCount = alte.length;
          ...
          child: Container(
              ...
              child: Column(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(height: 10,),
                    Text(
                        "Viziunea de dezvoltare personală",
                        style:
                    ...
                    RaisedButton(
                      child: Stack(
                        children: <Widget>[
                          Align(
                              alignment: Alignment.center,
                              child: Text(StrUserType == UserType.Student || StrUserType == UserType.Pupil?
                              "Salveaza si mergi la pasul 8":"Corect si mergi la pasul 8", style: TextStyle(fontFamily: 'regular')
                              )
                          )
                        ],
                      ),
                      onPressed: (){
                        if(StrUserType == UserType.Student || StrUserType == UserType.Pupil)
                        {
                          if (createPayload() == true)
                          {
//                            _apiTo_addCareerPlan7().then((res) {
//                              if (res == true)
//                              {
                                setState(() {
