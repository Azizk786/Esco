...
class CareerPlan_3 extends StatefulWidget {
  @override
  UserType  StrUserType;
  Map cp2;
  CareerPlan_3({Key key, @required this.StrUserType,this.cp2}) : super(key: key);
  _CareerPlan_3State createState() => _CareerPlan_3State(StrUserType,cp2);
}

class _CareerPlan_3State extends State<CareerPlan_3> {
  double _value = 0.7;
  UserType  StrUserType;
  Map cp2;
  //Map _payload;
  ...
  _CareerPlan_3State(this.StrUserType,this.cp2);
  ...
  SaveCreerPlan_Puncte_Slabe() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("CP3", json.encode(_cp3));
  }
  ...
  Future<Null> GetCVPlandata1() async
  {
    ...
    Map plandata = json.decode(
        sem_data["puncte_slabe"].replaceAll(new RegExp(r"(\s\n)"), ""));
    ...
    setState(() {
      ...
      _q4.text = plandata["1550836415479"]["experienta"];
    ...
    if(rowCount-1 == _inx)
    {
      return TextFormField(
        ...
        decoration: InputDecoration(
            labelText: 'Adaugă alte puncte slabe',
                  ...
                  children: <Widget>[
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        ...
                        SizedBox(height: 20,),
                        Text("Ce trasaturi de personalitate te împiedica să progresezi?",
                            style:
                            TextStyle(color: Colors.grey, fontSize: 16.0, fontFamily: 'regular')),
                        TextFormField(
                          maxLines: 5,
                        ...
                        SizedBox(height: 20,),
                        Text("Care sunt prejudecatile tale?",
                            style:
                            TextStyle(color: Colors.grey, fontSize: 16.0, fontFamily: 'regular')),
                        TextFormField(
                          maxLines: 5,
                        ...
                        SizedBox(height: 20,),
                        Text("Ce trebuie evitat din experienta anterioara negativa?",
                            style:
                            TextStyle(color: Colors.grey, fontSize: 16.0, fontFamily: 'regular')),
                        TextFormField(
                          maxLines: 5,
                    ...
                    RaisedButton(
                      ...
                      onPressed: (){
                        if(StrUserType == UserType.Student || StrUserType == UserType.Pupil)
                        {
                          if (createPayload() == true)
                          {
//                            _apiTo_addCareerPlan3().then((res) {
//                              if (res == true)
//                              {
                                setState(() {
                                  Navigator.push(context,
                                      MaterialPageRoute(builder: (context) => CareerPlan_4(StrUserType: StrUserType,cp3: _cp3,)));
                                });
//                              }
//                            });
                          }
