...
class LetterofIntent_4 extends StatefulWidget {
  @override
  UserType  StrUserType;
  LetterofIntent_4({Key key, @required this.StrUserType}) : super(key: key);
  _LetterofIntent_4State createState() => _LetterofIntent_4State(StrUserType);
}

class _LetterofIntent_4State extends State<LetterofIntent_4> {
  UserType  StrUserType;
  ...
  String struserid;
  Map userdata;
  List<String> _users = ["Stimata Doamna","Stimate Domnule","Stimata Doamna/Stimate domnule"];

  List<DropdownMenuItem<_FormulaType>> _dropdownMenuItems;
  final _Nume = TextEditingController();
  _LetterofIntent_4State(this.StrUserType);
...
//  }
  ...
  SaveLetterofIntent_PersonalInfo() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString("LOI4", json.encode(_loi4));
  }
  ...
  bool createPayload()
  {
    if(_selectedTitleType.length <1) {
      ...
      showMyDialog(context,"Introduceți Formula de salut");
      return false;
    }
  ...
  Map  Sem_Payload = {
  "titlu":_selectedTitleType,
  "nume":_Nume.text.toString(),
  };
    _loi4 = {
      "1566911866234":Sem_Payload,
    };
  ...
  Future<Null> GetCVPlandata() async
  {
    ...
    setState(() {
      ...
      Map  plandata = json.decode(
          sem_data["formula_salut"].replaceAll(new RegExp(r"(\s\n)"), ""));
      _Nume.text = plandata["1566911866234"]["nume"];
      _selectedTitleType = plandata["1566911866234"]["titlu"].toString();
...
//      ),
                ...
                child: Stack(
                  children: <Widget>[
                    Align(
                        alignment: Alignment.center,
                        child: Text(StrUserType == UserType.Student || StrUserType == UserType.Pupil?
                        "Salveaza si mergi la pasul 5":"Corect si mergi la pasul 5", style: TextStyle(fontFamily: 'regular')
                        )
                    )
                  ],
                ),
    ...
    MaterialPageRoute(builder: (context) => LetterofIntent_5(StrUserType: StrUserType)));
    }
                },
...
class _FormulaType {
  String name;
  _FormulaType(this.name);
  static List<_FormulaType> getTitle() {
    return <_FormulaType>[
      _FormulaType('Stimata Doamna'),
      _FormulaType('Stimate Domnule'),
      _FormulaType('Stimata Doamna/Stimate domnule'),
    ];
  }
}