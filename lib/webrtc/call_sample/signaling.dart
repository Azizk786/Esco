...
class Signaling {
  ...
  static List<dynamic> sharedPeers;
  var description;
  ...
  static Signaling getSignalingContext() {
    return signaling;
  }
...
//    this._sessionId = this._selfId.toString() + '-' + peer_id;
//
//    peerId = peer_id;
//
      ...
      case 'offer':
        {
          ...
          if(media.toString() == "video") {
            if (this.onStateChange != null) {
              this.onStateChange(SignalingState.CallStateNew);
            }
          } else if(media.toString() == "Ringing"){
            if (this.onStateChange != null) {
...
//      }
  ...
  _createOffer(String id, RTCPeerConnection pc, String media, String offerState) async {
    try {
      ...
      _send('offer', {
        ...
        'session_id': this._sessionId,
        'media': offerState == "Ringing" ? "Ringing" : media,
        'CallState': offerState
      });
    } catch (e) {
      print(e.toString());
    }
  ...
  callAccept(String peer_id, String media, use_screen) async {
    ...
    var pc = await _createPeerConnection(peer_id, media, false);
    _peerConnections[peer_id] = pc;
    await pc.setRemoteDescription(new RTCSessionDescription(
        description['sdp'], description['type']));
    await _createAnswer(peer_id, pc, media);
...
//        _createDataChannel(peer_id, pc);
//      }
//      _createOffer(peer_id, pc, media, "CallStarted");
//    });
  }

  void ring(String peer_id, String media, use_screen) {
    this._sessionId = this._selfId.toString() + '-' + peer_id;
    ...
    _createPeerConnection(peer_id, media, use_screen).then((pc) {
      ...
      _createOffer(peer_id, pc, media, "Ringing");
    });
  }
